...
// 進捗バー・バナー表示（株価の「最新株価取得」と同じ仕組み。setBulkUpdateBanner/Progressは汎用的なDOM操作のため
// price-update-all-*要素を直接参照しており、IRBANK側では同じ関数を使い回せない。専用の関数を用意する）
function setIrbankUpdateBanner(state, text) {
    const el = document.getElementById('irbank-update-all-status');
    el.textContent = text;
    el.classList.remove('update-banner--running', 'update-banner--success', 'update-banner--failure');
    if (state) el.classList.add(`update-banner--${state}`);
}

function setIrbankUpdateProgress(processed, target) {
    const wrap = document.getElementById('irbank-update-all-progress');
    const bar = document.getElementById('irbank-update-all-bar');
    const percentEl = document.getElementById('irbank-update-all-percent');
    const pct = target > 0 ? Math.min(100, Math.round((processed / target) * 100)) : 0;
...
let irbankUpdateTrackingGen = 0; // ボタン多重クリック時、古いポーリングを打ち切るための世代カウンタ

// baselineRun/baselineCommit: 起動直前（dispatchWorkflowを呼ぶ前）に取得しておいた「それまでの最新」の状態。
// 株価の一括取得（trackBulkUpdateProgress）と同じ考え方：時刻ではなくid/shaの変化で新しい実行・コミットを判定する。
async function trackIrbankUpdateProgress(myGen, baselineRun, baselineCommit, targetCount, btn) {
    const token = getTokenValue();
    const baselineRunId = baselineRun ? baselineRun.id : null;
    const baselineCommitSha = baselineCommit ? baselineCommit.sha : null;
    ...
    let lastSeenRunId = null;
    for (let i = 0; i < 10; i++) {
        if (myGen !== irbankUpdateTrackingGen) return; // 別の実行が始まっていたら中断
        try {
            const latest = await getLatestWorkflowRun(token, OWNER, CODE_REPO, IRBANK_WORKFLOW_FILE);
            lastSeenRunId = latest ? latest.id : null;
            lastError = null;
            if (latest && latest.id !== baselineRunId) run = latest;
        } catch (error) {
    ...
    while (myGen === irbankUpdateTrackingGen) {
        try {
            const latestRun = await getWorkflowRun(token, OWNER, CODE_REPO, run.id);

            const commit = await getLatestCommit(token, OWNER, DATA_REPO, DATA_REPO_BRANCH, IRBANK_PATH);
            if (commit && commit.sha !== baselineCommitSha) {
                const processed = parseProcessedFromCommitMessage(commit.message);
                if (processed !== null) setIrbankUpdateProgress(processed, targetCount);
            }

            if (latestRun.status === 'completed') {
                const ok = latestRun.conclusion === 'success';
                setIrbankUpdateProgress(targetCount, targetCount);
                setIrbankUpdateBanner(ok ? 'success' : 'failure',
                    ok
                        ? `完了しました（対象 ${targetCount}銘柄）。状態パネルを更新します...`
                        : `完了しましたが、一部失敗した可能性があります（結果: ${latestRun.conclusion}）。詳細はGitHubのActionsタブで確認してください。`
                );
                btn.disabled = false;
                loadIrbankStatus();
...
document.getElementById('irbank-update-all-btn')?.addEventListener('click', async (event) => {
    const btn = event.currentTarget;
    const token = getTokenValue();
    if (!token) { alert('トークンを入力してください'); return; }

    btn.disabled = true;
    document.getElementById('irbank-update-all-progress').style.display = 'none';
    setIrbankUpdateBanner('running', '対象銘柄数を確認中...');
    ...
    try {
        ...
        if (targetCount === 0) {
            setIrbankUpdateBanner('failure', '対象銘柄が0件でした。ワークフローは起動していません。');
            btn.disabled = false;
            return;
        }

        setIrbankUpdateBanner('running', '実行状況を確認中...');

        const [baselineRun, baselineCommit] = await Promise.all([
            getLatestWorkflowRun(token, OWNER, CODE_REPO, IRBANK_WORKFLOW_FILE).catch(() => null),
            getLatestCommit(token, OWNER, DATA_REPO, DATA_REPO_BRANCH, IRBANK_PATH).catch(() => null),
        ]);

        // 前回の実行がまだ動いている状態でもう一度起動すると、同じファイルへ同時にコミット・pushしようとして
        // 競合し、片方が失敗することがある（株価一括取得と同じ注意）。事前に警告し、続行するか確認する。
        if (baselineRun && (baselineRun.status === 'in_progress' || baselineRun.status === 'queued')) {
            const proceed = confirm(
                '前回の「IRBANK企業ID取得」がまだ実行中の可能性があります。\n' +
                '同時に実行すると、データリポジトリへのコミットが競合し、片方が失敗する場合があります。\n' +
                'それでも実行しますか？'
            );
            if (!proceed) {
                setIrbankUpdateBanner(null, '実行をキャンセルしました。前回の実行が完了してから再度お試しください。');
                btn.disabled = false;
                return;
            }
        }

        setIrbankUpdateBanner('running', '実行をリクエスト中...');

        await dispatchWorkflow(token, OWNER, CODE_REPO, IRBANK_WORKFLOW_FILE, CODE_REPO_BRANCH, {
            offset: '0', limit: String(targetCount)
        });

        setIrbankUpdateProgress(0, targetCount);
        setIrbankUpdateBanner('running', `実行をリクエストしました（対象 ${targetCount}銘柄）。実行状況を確認しています...`);

        irbankUpdateTrackingGen += 1;
        trackIrbankUpdateProgress(irbankUpdateTrackingGen, baselineRun, baselineCommit, targetCount, btn);
    } catch (error) {
        console.error(error);
        setIrbankUpdateBanner('failure', `失敗しました: ${error.message}`);
        btn.disabled = false;
    }
});

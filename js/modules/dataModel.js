...
export const MAIN_DATA_COLUMNS = [
    ...
    'カテゴリ', 'タグ', 'ハブ', 'ステータス', 'ステータスコメント', '優先度',
    '期限', '完了日', '見積時間', '実績時間', 'タイムスタンプログ', '補正時間',
    '繰返し頻度', '繰返し頻度_詳細', 'Input', 'Output'
];

export const MASTER_DATA_COLUMNS = [
    '(M)変数名', '(M)変数分類', '(M)変数説明', '(M)データ区分', '(M)カテゴリ',
    '(M)タグ_親', '(M)タグ_子', '(M)ハブ_親', '(M)ハブ_子',
    '(M)ステータス_親', '(M)ステータス_子', '(M)優先度',
    '(M)Input', '(M)Output', '(M)繰返し頻度', '(M)繰返し頻度_詳細'
];
